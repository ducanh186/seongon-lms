...
export { resolveMaterialUrl } from './apiUrl';
...
export const api = {
  ...
  downloadAdminReport: async (token: string, report: 'enrollments' | 'completion' | 'courses' | 'popular-courses' | 'revenue') => {
    ...
    if (!response.ok) {
      throw new ApiError('Không thể tải báo cáo.', response.status);
    }
  ...
  saveQuiz: (token: string, courseId: number, body: { title: string; pass_score: number; max_attempts: number; duration_minutes?: number | null }) =>
