  ...
  return <Stack spacing={2}>
    ...
    <Card><CardContent>
      <Box component="form" onSubmit={(event) => void save(event)}>
        <Typography component="h2" variant="h6">{editing ? 'Sửa danh mục tin tức' : 'Tạo danh mục tin tức'}</Typography>
        <Stack spacing={2} sx={{ mt: 2 }}>
          <TextField required label="Tên danh mục tin tức" value={name} onChange={(event) => setName(event.target.value)} inputProps={{ maxLength: 100 }} />
          <TextField label="Mô tả danh mục" multiline minRows={2} value={description} onChange={(event) => setDescription(event.target.value)} inputProps={{ maxLength: 2000 }} />
          <Stack direction="row" spacing={1}><Button type="submit" variant="contained" disabled={busy || !name.trim()}>{editing ? 'Cập nhật danh mục' : 'Lưu danh mục tin tức'}</Button>{editing && <Button disabled={busy} onClick={reset}>Hủy sửa</Button>}</Stack>
        </Stack>
      </Box>
    </CardContent></Card>
    <Card>{loading ? <Typography role="status" sx={{ p: 3 }}>Đang tải danh mục tin tức…</Typography> : <AdminDataTable<ApiCatalog>
      label="Danh sách danh mục tin tức" rows={rows} getRowKey={(row) => row.id} minWidth={480}
      columns={[
        { key: 'name', header: 'Danh mục', render: (row) => row.name },
        { key: 'description', header: 'Mô tả', render: (row) => row.description || '—' },
        { key: 'actions', header: 'Thao tác', align: 'center', render: (row) => <Stack direction="row" justifyContent="center"><Button disabled={busy} onClick={() => { setEditing(row); setName(row.name); setDescription(row.description ?? ''); }}>Sửa</Button><Button disabled={busy} color="error" onClick={() => setRemoving(row)}>Xóa</Button></Stack> },
      ]}
    />}</Card>
