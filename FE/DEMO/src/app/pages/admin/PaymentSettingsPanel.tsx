...
export function PaymentSettingsPanel({ token }: { token: string }) {
  ...
  return <Box component="form" onSubmit={(event) => { event.preventDefault(); void save(); }}><Stack spacing={3}>
    ...
    <Card variant="outlined"><CardContent><Stack spacing={2}>
      ...
      <Typography variant="body2" color="text.secondary">Bật thanh toán ngân hàng để hiển thị phương thức này cho học viên và cho phép tài khoản nhận thanh toán.</Typography>
    ...
    {savedBank?.bank_name && savedBank.account_number && <Card variant="outlined"><CardContent><Stack spacing={1}>
      ...
      <Typography color={savedBank.enabled ? 'success.main' : 'text.secondary'}>{savedBank.enabled ? 'Đang nhận thanh toán' : 'Chưa nhận thanh toán'}</Typography>
    </Stack></CardContent></Card>}
  </Stack></Box>;
}
