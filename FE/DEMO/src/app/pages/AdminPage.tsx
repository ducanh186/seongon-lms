...
const QUIZ_QUESTIONS_PER_ATTEMPT = 40;
    ...
    try {
      switch (section) {
        ...
        case 'courses': {
          ...
          setInstructors(nextInstructors.data);
          break;
        }
        case 'reviews': {
    ...
    <Card component="form" onSubmit={submitCourse} sx={{ borderRadius: 3 }}>
      <CardContent>
        <Stack spacing={2}>
          ...
          <FormControl>
            <InputLabel id="course-instructor">Người biên soạn chương trình học</InputLabel>
            <Select labelId="course-instructor" label="Người biên soạn chương trình học" value={courseForm.teacher_profile_id === null ? '' : String(courseForm.teacher_profile_id)} onChange={(event) => {
              const value = event.target.value;
              setCourseForm((form) => ({ ...form, teacher_profile_id: value === '' ? null : Number(value) }));
            }}>
              <MenuItem value="">Chưa chọn người biên soạn</MenuItem>
              {instructors.map((instructor) => <MenuItem key={instructor.id} value={String(instructor.id)}><Stack direction="row" spacing={1} alignItems="center"><Avatar src={resolveMaterialUrl(instructor.avatar ?? undefined)} alt={`Ảnh người biên soạn chương trình học ${instructor.name}`} sx={{ width: 28, height: 28 }} /><span>{instructor.name}</span></Stack></MenuItem>)}
              {instructors.length === 0 && <MenuItem value="" disabled>Chưa có người biên soạn chương trình học</MenuItem>}
            </Select>
          </FormControl>
          {selectedTeacherProfile && <Card variant="outlined" sx={{ borderRadius: 2 }}>
            <CardContent>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }}>
                <Avatar src={resolveMaterialUrl(selectedTeacherProfile.avatar ?? undefined)} alt={`Ảnh hồ sơ người biên soạn chương trình học ${selectedTeacherProfile.name}`} sx={{ width: 72, height: 72 }}>{selectedTeacherProfile.name[0]}</Avatar>
                <Stack spacing={1} sx={{ flexGrow: 1 }}>
                  <Typography fontWeight={800}>{selectedTeacherProfile.name}</Typography>
                  <TextField label="Giới thiệu người biên soạn chương trình học" value={selectedTeacherProfile.bio ?? ''} multiline minRows={2} fullWidth InputProps={{ readOnly: true }} />
                </Stack>
              </Stack>
            </CardContent>
          </Card>}
                ...
                <CardContent sx={{ p: { xs: 2.5, md: 3 }, '&:last-child': { pb: { xs: 2.5, md: 3 } } }}>
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))', lg: 'repeat(3, minmax(0, 1fr))' }, gap: 2.5 }}>
                    {[
                      ...
                      ['Người biên soạn chương trình học', selectedCourse.teacher_profile?.name || '—'],
                  ...
                  {selectedCourse.quiz ? (
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))', lg: 'repeat(4, minmax(0, 1fr))' }, gap: 2.5, mt: 2 }}>
                      {[
                        ...
                        ['Câu mỗi lượt', String(QUIZ_QUESTIONS_PER_ATTEMPT)],
