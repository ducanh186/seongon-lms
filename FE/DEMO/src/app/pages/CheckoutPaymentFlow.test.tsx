...
beforeEach(() => {
  ...
  mocks.paymentMethods.mockResolvedValue({ data: [{ code: 'momo', label: 'Thanh toán qua ví MoMo', mode: 'mock' }, { code: 'bank', label: 'Thanh toán qua ngân hàng', mode: 'mock' }, { code: 'card', label: 'Thanh toán bằng thẻ', mode: 'mock' }] });
...
it('requires a card type before confirming a mock card payment', async () => {
  ...
  const user = userEvent.setup();
  await user.click(await screen.findByRole('button', { name: 'Lưu thông tin và tạo đơn' }));
  await user.click(await screen.findByRole('radio', { name: 'Thanh toán bằng thẻ' }));
  await user.click(screen.getByRole('button', { name: 'Tiếp tục' }));
