...
const mocks = vi.hoisted(() => ({ course: vi.fn(), createOrder: vi.fn(), paymentMethods: vi.fn(), getOrder: vi.fn(), startPayment: vi.fn(), mockPaymentCallback: vi.fn(), updateProfile: vi.fn(), refreshUser: vi.fn(), refresh: vi.fn() }));
...
it('starts a real API session on Continue and only confirms through the mock callback', async () => {
  ...
  expect(screen.queryByTestId('card-payment-logo')).not.toBeInTheDocument();
...
it('does not offer card payment even if a stale methods response includes it', async () => {
  mocks.paymentMethods.mockResolvedValue({ data: [
    { code: 'card', label: 'Thanh toán bằng thẻ', mode: 'mock' },
    { code: 'momo', label: 'Thanh toán qua ví MoMo', mode: 'mock' },
  ] });
  render(<MemoryRouter><CheckoutPage /></MemoryRouter>);
  const user = userEvent.setup();
  ...
  expect(screen.queryByRole('radio', { name: 'Thanh toán bằng thẻ' })).not.toBeInTheDocument();
  expect(screen.getByRole('radio', { name: 'Thanh toán qua ví MoMo' })).toBeInTheDocument();
});

it('lets the student cancel an existing card session without showing card payment controls', async () => {
  mocks.getOrder.mockResolvedValue({ data: {
    ...order,
    payment_method: 'card',
    payment_status: 'pending',
    payment_expires_at: new Date(Date.now() + 900000).toISOString(),
    payment_session: { token: 'old-card-session', mode: 'mock', reference: 'LMS-44', merchant_name: 'SEONGON', qr_payload: null, bank: null },
    mock_callback_allowed: true,
  } });
  render(<MemoryRouter initialEntries={['/checkout/seo?order=44']}><CheckoutPage /></MemoryRouter>);

  expect(await screen.findByText(/Phiên thanh toán cũ không còn được hỗ trợ/)).toBeInTheDocument();
  expect(screen.queryByRole('heading', { name: 'Chọn loại thẻ' })).not.toBeInTheDocument();
  expect(screen.queryByRole('button', { name: 'Xác nhận thanh toán' })).not.toBeInTheDocument();
  expect(screen.getByRole('button', { name: 'Hủy phiên thanh toán' })).toBeInTheDocument();
});
