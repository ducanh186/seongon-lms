...
export function LearnCoursePage() {
  ...
  return <Box sx={{ py: { xs: 3, md: 5 }, bgcolor: '#f7fafb', minHeight: '70dvh' }}><Container maxWidth="lg"><Stack spacing={3}>
    <Box><Typography component="h1" variant="h4" fontWeight={800}>{enrollment?.course?.title ?? 'Học khóa học'}</Typography><Typography color="text.secondary" sx={{ mt: .5 }}>Hoàn thành từng bài học để mở bài kiểm tra cuối khóa.</Typography></Box>
    ...
    <LinearProgress variant="determinate" value={progress?.percent ?? 0} sx={{ height: 10, borderRadius: 5 }} />
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 1.6fr) minmax(300px, .75fr)' }, gap: 3, alignItems: 'start' }}>
      <Card sx={{ borderRadius: 3 }}><CardContent><Stack spacing={2}>
        <Typography component="h2" variant="h6" fontWeight={800}>{activeLesson?.title ?? 'Chọn một bài học'}</Typography>
        {activeLesson?.video_url ? <Box component="iframe" src={activeLesson.video_url} title={activeLesson.title} sx={{ width: '100%', aspectRatio: '16 / 9', border: 0, borderRadius: 2 }} allowFullScreen /> : <Alert severity="info">Bài học này chưa có video.</Alert>}
      ...
      <Card sx={{ borderRadius: 3 }}><CardContent><Typography component="h2" variant="h6" fontWeight={800}>Danh sách bài học</Typography><Stack divider={<Divider flexItem />} sx={{ mt: 1 }}>
        {lessons.map((lesson) => <Button key={lesson.id} onClick={() => setActiveLesson(lesson)} color="inherit" sx={{ justifyContent: 'flex-start', textAlign: 'left', py: 1.5, fontWeight: lesson.id === activeLesson?.id ? 800 : 400 }} startIcon={lesson.is_completed ? <CheckCircleOutlineIcon color="primary" /> : undefined}>{lesson.position}. {lesson.title}</Button>)}
      </Stack></CardContent></Card>
    </Box>
    <Card sx={{ borderRadius: 3 }}><CardContent><Stack spacing={2}><Typography component="h2" variant="h5" fontWeight={800}>Bài kiểm tra cuối khóa</Typography>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3 }}>
      <Card sx={{ borderRadius: 3 }}><CardContent><Stack spacing={2}><Typography component="h2" variant="h6" fontWeight={800}>Đánh giá khóa học</Typography><Rating value={rating} onChange={(_, value) => setRating(value)} /><TextField label="Nhận xét của bạn" multiline minRows={3} value={comment} onChange={(event) => setComment(event.target.value)} /><Button variant="outlined" onClick={() => void submitReview()} disabled={!rating}>Gửi đánh giá</Button></Stack></CardContent></Card>
      <Card sx={{ borderRadius: 3 }}><CardContent><Stack spacing={2}><Typography component="h2" variant="h6" fontWeight={800}>Chứng chỉ</Typography><Typography color="text.secondary">Sau khi đạt bài kiểm tra, bạn có thể tải chứng chỉ PDF tại đây.</Typography><Button variant="outlined" startIcon={<DownloadOutlinedIcon />} onClick={() => void downloadCertificate()}>Tải chứng chỉ</Button></Stack></CardContent></Card>
    </Box>
  </Stack></Container></Box>;
}
