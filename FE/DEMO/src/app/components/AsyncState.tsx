...
export function PageSkeleton({ rows = 3 }: { rows?: number }) {
  return (
    <Stack spacing={2} aria-label="Đang tải nội dung" role="status" aria-live="polite">
...
export function EmptyState({ title, action }: { title: string; action?: ReactNode }) {
  return <Box sx={{ p: 3, border: '1px dashed', borderColor: 'divider', borderRadius: 3, bgcolor: 'background.paper', textAlign: 'center' }}><Typography color="text.secondary">{title}</Typography>{action && <Box sx={{ mt: 2 }}>{action}</Box>}</Box>;
}
